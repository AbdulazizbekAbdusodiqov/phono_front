...
function LocationMarker({ setPosition, updateAddress }: { 
...
export default function MapContent({ addressData, setAddressData }: MapContentProps) {
  const [position, setPosition] = useState<[number, number] | null>(
    addressData?.lat && addressData?.long 
      ? [parseFloat(addressData.lat), parseFloat(addressData.long)]
      : null
  );
  const [address, setAddress] = useState<string>(addressData?.address || 'Manzil yuklanmoqda...');
  const [locationName, setLocationName] = useState<string>(addressData?.name || '');
      ...
      if (data.display_name) {
        ...
        if (data.address.road) {
          ...
          if (data.address.house_number) {
            addressParts[addressParts.length - 1] += ` ${data.address.house_number}`;
          }
        }
        ...
        <Popup>
          ...
          <div className="text-sm">
            ...
            <div className="text-xs text-gray-500">
              Kenglik: {position[0].toFixed(6)}°N, Uzunlik: {position[1].toFixed(6)}°E
            </div>
          </div>
