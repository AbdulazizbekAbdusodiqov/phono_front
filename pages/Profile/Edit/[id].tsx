...
// import { editProduct } from "@/hooks/products.use"
...
const EditProduct = () => {
  ...
  console.log("productData: ", productData2, id)
  ...
  useEffect(() => {
    if (productData2?.product_image) {
      const imageUrls = productData2.product_image.map((img: any) => img.url)
  ...
  useEffect(() => {
    ...
    if(productData2){
      if(productData2?.user_id != user?.id) {
        router.push("/Profile")
      }
    }    
    if (isAuthenticated) {
